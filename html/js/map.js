...
*/
    ...
    $.ajax({
	    ...
	    console.log('FIRST SAFESPOT: ' + spots[0].lat);
            safespotsLen = spots.length;
            if (safespotsLen != 0) {
                for (var x = 0; x < safespotsLen; x++) {
		    var i = map.markers.length;
		    var content = template.replace(/{{address}}/g, spots[x].name).replace(/{{index}}/g, i).replace(/{{lat}}/g, spots[x].lat).replace(/{{lng}}/g, spots[x].lng);
                    //gMarkers['safespots'];
	    ...
	    console.log('gMarkers: ' + gMarkers['safespots'][0].title);
            }
            else {
	...
	console.log(json_data);
        $.ajax({
            ...
            success: function (data) {
                ...
                var template = $('#edit_marker_template').html();
                if (crimesLen > 0) {
                    $('#crime').prepend('<h4>Crimes near your location:</h4>');
                    for (var x = 0; x < crimesLen; x++) {
//____
                        ...
                        GMaps.geocode({
                            ...
                            callback: function(results, status) {
                                if (status == 'OK') {
                                    ...
                                    a1lat = latlng.lat();
                                    a1lng = latlng.lng();
                                    var i = x+1;//;map.markers.length;
                                    var content = template.replace(/{{address}}/g, crimeAddress).replace(/{{index}}/g, i).replace(/{{lat}}/g, a1lat).replace(/{{lng}}/g, a1lng);
                                    console.log("Coords, array", a1lat, a1lng, i, gMarkers['crimes'][x]);
                                    gMarkers['crimes'].push({lat:a1lat, lng:a1lng, title: crimeAddress, infoWindow: {content: content}, icon: 'red_exclamation_icon.png'
                                    });
                                }
                            }
...
//___________________________
    ...
    $(".toggle_markers > label > input:checkbox").click(function() {
   ...
   //3.) SEARCH FOR A PLACE BY SPECIFYING THE ADDRESS:
    ...
    $(document).on('click', '#b_route_safespot', function(e){
        ...
        if(safespots.length > 0){
            ...
            var points, minimum, min2, min3,index, index2, index3, index1, p1, p2, p3, dc1, dc2, dc3, dc1c=0, dc2c=0, dc3c=0;
            var route, mincount;
            var d = [];
            closeby=[];
            closeby1=[];
    ...
    $(document).on('click', '#make_safespot', function(e) {
        ...
        if(exists != true) {
            ...
            geocoder.geocode({'location': latlng}, function (results, status) {
                ...
                new_spot = {name: new_address, lat: latlng.lat, lng: latlng.lng};
