...
$(document).ready(function(){
    ...
    $.ajax({
        ...
        url: '/api/spots',
        success: function(res) {
            var safespots = $.makeArray(res);
            safespotLen = JSON.parse(safespots[0]).length;
	    console.log(JSON.parse(safespots[0]));
	    safespots = JSON.parse(safespots[0]);
            if(safespotLen > 0) {
                for (i = 0; i < safespotLen; i++) {
		    global_spots_array.push(safespots[i]);		
    ...
    $.ajax({
	    ...
	    ecInfo = {contacts : data}; 
            ecLen = ecInfo["contacts"].length;
            if(ecLen != 0)
            {
                for(var x = 0; x < ecLen; x++ ){
                    $("#ec").append('<li value="'+ ecInfo["contacts"][x].phone +'">'+ ecInfo["contacts"][x].firstname +' '
                      + ecInfo["contacts"][x].lastname + '</li><p style="display: inline" value="'+ ecInfo["contacts"][x].phone
                      +'" class="deleteEC" id="'+ x +'">(-)</p> <p style="display: inline" value="'+ ecInfo["contacts"][x].phone
                      + '" name="' + ecInfo["contacts"][x].firstname +'" class="editEC" id="'+ x +'">(edit)</p><br>');
	...
	*/ 
    ...
    $("#ec").on('click', ".deleteEC", function(){
        ...
        for(var i = 0; i < ecLen; i++ ){
            if(parseInt(ecInfo["contacts"][i].phone) === pnum1){
                ...
                ecFirst = ecInfo["contacts"][i].firstname;
                ecLast = ecInfo["contacts"][i].lastname;
    ...
    $("#ul_safespots").on('click', ".deleteSS", function(){
        ...
        for(var i = 0; i < safespotLen; i++ ){
            if(global_spots_array[i].name === name){
                ssName = global_spots_array[i].name;
                ssLat = global_spots_array[i].lat;
                ssLng = global_spots_array[i].lng;
            }
        }
